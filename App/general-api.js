...
//var smtpTransport = nodemailer.createTransport("smtps://miguelarcangel96@gmail.com:"+encodeURIComponent('Siemens123.abc$') + "@smtp.gmail.com:465");

        //var mailOptions={
            let HelperOptions={
            from:"'Asignacion Siemens' <asignacionsiemens@gmail.com",
            to: "miguelarcangel96@gmail.com",
            subject: "¡Nueva asignación!",
            text:"Tiene una nueva asignación por favor, abrir la apliación"
        }
    ...
    con.query(queryDuplicado, (error, result) => {
        if(error){
            ...
            if(result.length == 0){
                ...
                auxCertificadoA.saveCertificadoA(certificadoPathMD, base64CertificadoMD).then((CertificadoAResultMD) => {
...
router.get("/getReportByAssignment/:id", (req, res) => {
    ...
    let query = "SELECT RG.Consecutivo, RG.FechaEnvio, RG.NombreContacto, RG.NombreColaborador, RG.NombreProyecto, RG.DescripcionAlcance, RG.Marca, RG.DenominacionInterna, RG.NumeroSerial, RG.CaracteristicasTecnicas, RG.EstadoInicial, RG.ActividadesRealizadas, RG.Conclusiones, RG.RepuestosSugeridos, RG.ActividadesPendientes, RG.HojaTiempo, RG.FirmaEmisor, RG.FirmaCliente, RG.Adjuntos, E.NombreEmpresa, T.CostoViaje, T.ervicio FROM ReporteGeneral AS RG INNER JOIN Empresa AS E ON RG.IdEmpresa=E.IdEmpresa INNER JOIN Asignacion ON RG.IdAsignacion=Asignacion.IdAsignacion INNER JOIN Especialista ON Asignacion.IdEspecialista=Especialista.IdEspecialista INNER JOIN Tecnica AS T ON Especialista.IdTecnica=T.IdTecnica WHERE RG.IdAsignacion=" + IdAsignacion + ";";
